; Production Systems
; Marcus Vinicius Pinto Pireira Jr. and Jack Beal
; 2017-11-06
...
(define rules
  '(
    ; Move to a clear adjacent space
    (r1 (if (current x) (adjacent y) (not path x y) (not beenHereBefore y) (not visited y) (not obstacle y))
        (delete (current x))
        (add (path y x) (beenHereBefore x) (delete_adjacents) (move_to y)))

    ; Move to a low stable adjacent obstacle
    (r2 (if (current x) (adjacent y) (not path x y) (not beenHereBefore y) (not visited y) (obstacle y) (height y low) (stable y) (not move_to y))
        (delete (current x))
        (add (path y x) (beenHereBefore x) (delete_adjacents) (move_to y)))

    ; Backtrack along the path already taken
    (r3 (if (current x) (adjacent y) (path x y) (not visited y) )
        (delete (current x) (beenHereBefore x) (path x y))
        (add (visited x) (delete_adjacents) (backtrack_to y)))

    ; Clear out facts related to adjacent nodes
    (r4 (if (delete_adjacents) (adjacent x))
        (delete (adjacent x)))

    ; Remove delete_adjacents flag
    (r5 (if (delete_adjacents))
        (delete (delete_adjacents)))

    ; Execute the move_to program
    (r6 (if (move_to x) (not delete_adjacents))
        (delete (move_to x))
        (execute (move_to x)))

    ; Execute the backtracking program
